  ...
  return (
    <nav className="w-full border-b px-4 py-2.5 md:px-8" id="home">
      <div className="flex items-center justify-between gap-4">
        ...
        >
          <div className="border-primary/50 group-hover:border-primary absolute -top-2 -left-2 h-4 w-4 border-t-2 border-l-2 duration-200 group-hover:-top-1 group-hover:-left-1" />
          <Logo className="w-14" hover />
          <div className="border-primary/50 group-hover:border-primary absolute -right-2 -bottom-2 h-4 w-4 border-r-2 border-b-2 duration-200 group-hover:-right-1 group-hover:-bottom-1" />
        </a>

        {/* Desktop Nav */}
        <div className="font-incognito relative hidden items-center md:flex">
          {/* Overlay layer  */}
          <div
            ref={overlayRef}
          ...
          >
            <div className="bg-primary/20 relative flex gap-1 rounded-full border border-primary/30 px-2 py-1 shadow-[0_0_8px_theme(colors.primary/0.5)]">
              {NAV_LINKS.map((x) => (
                <div
                  key={x.id}
              ...
              return (
                <a
                  ...
                  className={cn(
                    ...
                    isActive ? "text-primary" : "opacity-70 hover:opacity-100",
                  )}
                >
                  {x.label}
                </a>
        ...
        <div className="inline-flex items-center gap-3">
          <div className="inline-flex items-center gap-2 rounded-full border bg-background/50 px-3 py-1.5 backdrop-blur-sm">
            {/* GitHub */}
            <a
            ...
            <AnimatedThemeToggler className="transition-transform duration-200 hover:scale-110" />
          </div>

          {/* Mobile menu button */}
          <button
            className="hover:bg-accent inline-flex size-9 items-center justify-center rounded-md border transition-colors md:hidden"
            onClick={() => setOpen((s) => !s)}
        ...
        {open && (
          ...
          >
            <motion.div
              ...
              animate={{ y: 0 }}
              exit={{ y: -20 }}
              className="mt-2 grid gap-1 rounded-xl border bg-background/50 p-2 backdrop-blur-sm"
            >
              {NAV_LINKS.map((x, index) => (
                ...
                >
                  ...
                  <div className="from-primary/0 via-primary/10 to-primary/0 absolute inset-0 translate-x-[-100%] bg-gradient-to-r transition-transform duration-700 ease-in-out group-hover:translate-x-[100%]" />
